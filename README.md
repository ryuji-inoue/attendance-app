...
```bash
# リポジトリのクローン
git clone https://github.com/ryuji-inoue/attendance-app.git  .
...
###  レイアウト変更
・バグに繋がりかねないUIや、利便性を損なう仕様は下記の通り修正。

    ・メール認証待ちの画面でも「ログアウト」だけは表示させる。
        ユーザーがメールアドレス誤登録した場合に、ログアウトできず、画面に閉じ込められるバグを懸念。

    ・ロゴをクリックするとトップページに戻る。
        Webデザインの標準的な仕様であり、仕様書には記載がないが、デファクトスタンダードとして省略されたものと判断。

###  仕様の矛盾について
・テストケースと機能要件、画面デザインで矛盾しているケースについては、下記のように判断。
...
### 要件に対してテストの網羅性が不足している項目
    機能要件で定義されている条件に対して、テストケース側の検証パターンが足りていない項目。
